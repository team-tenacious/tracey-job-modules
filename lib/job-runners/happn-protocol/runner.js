...
HappnProtocolRunner.prototype.start = function (callback) {
    ...
    async.waterfall([
        function (cb1) {
            ...
            self.__archiver.createArchive(self.__currentJob.folder, tempFolder, 'happn-protocol', function (err, result) {
                if (err)
                    return cb1(err);
        ...
        function (archiveName, cb2) {
            // STEP 3: run Dockerfile and create image (repo is injected into container to run)
            self.__emit('building Docker image');

            var archivePath = path.join('tmp', archiveName);

            self.__dockeriser.buildImage(__dirname, ['Dockerfile', archivePath], 'happn-protocol', function (err) {
                if (err)
                    return cb2(err);
