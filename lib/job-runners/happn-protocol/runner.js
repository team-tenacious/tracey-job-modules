...
HappnProtocolRunner.prototype.start = function (callback) {
    ...
    async.waterfall([
        function (cb1) {
            ...
            self.__archiver.createArchive(self.__currentJob.folder, cb1);
        },
        function (cb2) {
            // STEP 3: run Dockerfile and create image (repo is injected into container to run)
            self.__emit('building Docker image');
            self.__dockeriser.buildImage(__dirname, 'Dockerfile', 'happn-protocol', cb2);
        },
        function (cb3) {
