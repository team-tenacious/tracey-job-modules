...
function HappnProtocolRunner(job, commander = null, giteriser = null) {
    ...
    console.log(this.__currentJob.folder);

    this.__commander = commander || new Commander();
    this.__giteriser = new Giteriser(this.__currentJob.folder);
...
HappnProtocolRunner.prototype.start = function (callback) {
    ...
    async.waterfall([
        ...
        function (cb3) {
            ...
            self.__giteriser.add('./*', function (err, result) {
                if (err) {
        ...
        function (cb4) {
            ...
            self.__giteriser.commit('protocol describe run', function (err, result) {
                if (err) {
                    console.log('ERROR: ', err);
                    cb4(err);
                }
        ...
        function (cb5) {
            ...
            self.__giteriser.status(function(err, result){
                ...
                self.__giteriser.push('origin', currentBranch, function (err, result) {
                    if (err) {
                        console.log('ERROR: ', err);
                        cb5(err);
                    }
