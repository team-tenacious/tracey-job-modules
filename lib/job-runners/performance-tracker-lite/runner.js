...
PerformanceTrackerRunner.prototype.start = function (callback) {
    ...
    if (self.__currentJob.message.npmInstall == null || self.__currentJob.message.npmInstall == true) {
        ...
        self.__commander.run('cd ' + self.__currentJob.message.folder + ' && npm install', function (err, result) {
            ...
            self.__runVersionTests(versions, state, errors, callback);
        })
    } else {    // bypass the npm install step as we already have the dependencies
        self.__runVersionTests(versions, state, errors, callback);
    }
};

PerformanceTrackerRunner.prototype.__runVersionTests = function (versions, state, errors, callback) {
