...
function PackagerRunner(job, commander, giteriser) {
    ...
    this.__giteriser = giteriser != null ? giteriser : new(require('../../utils/giteriser'))(this.__currentJob.folder);

    this.events = new EventEmitter();
}

PackagerRunner.prototype.start = function (callback) {
...
PackagerRunner.prototype.__updateState = function (state, message, context) {
    this.__emit('test-progress', {state: state, message: message, context: context});
};

PackagerRunner.prototype.__emit = function (evt, data) {
    return this.events.emit(evt, data);
};

PackagerRunner.prototype.on = function (evt, handler) {
    return this.events.on(evt, handler);
};

PackagerRunner.prototype.removeListener = function (evt, handler) {
